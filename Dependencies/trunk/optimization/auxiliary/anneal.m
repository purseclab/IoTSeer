...
%                  Default is 20.
%   MaxIterations: Maximum nb of iterations from start to finish
%
%
%  Usage:
...
elseif nargin<3, %user gave no options structure, use default
    ...
    fs = {'CoolSched','Generator','InitTemp','MaxConsRej',...
        'MaxSuccess','MaxTries','StopTemp','StopVal','Verbosity', 'MaxIterations'};
    for nm=1:length(fs)
...
% counters etc
itry = 0;  % nb of iterations at the current temperature
success = 0;
finished = 0;
consec = 0;
T = Tinit;
...
total = 0;
nbiterations = 0; % number of iterations from start to finish
if report==2, fprintf(1,'\n  T = %7.5f, loss = %10.5f\n',T,oldenergy); end

while ~finished;
    itry = itry+1; 
    nbiterations = nbiterations + 1;
    if mod(nbiterations,1)==0 % progress indicator
        disp(nbiterations)
    end
    if nbiterations >= options.MaxIterations
        break
    end
