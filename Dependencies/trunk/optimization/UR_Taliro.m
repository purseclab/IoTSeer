function [run, history] = UR_Taliro(inpRanges,opt)
...
if opt.search_space_constrained.constrained || opt.varying_cp_times>0
    error(' UR_Taliro does not support time varying control points or polyhedral search spaces. Use random walk through SA_Taliro instead.')
end
...
nSamples = params.n_tests;StopCond = opt.falsification;
