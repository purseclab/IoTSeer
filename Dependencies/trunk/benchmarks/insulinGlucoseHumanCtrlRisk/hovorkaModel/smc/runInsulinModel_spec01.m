...
clear;
    ...
    for i=1:max_n_tests/n_of_workers
         ...
         parfor ipar = 1:n_of_workers
            ...
            idx = find(sigout{ipar}.time>=240,1);
            rob(ipar) = min(sigout{ipar}.signals(1).values(idx,:)<=10);
            rob(ipar) = min(rob(ipar),min(sigout{ipar}.signals(1).values<=25));
