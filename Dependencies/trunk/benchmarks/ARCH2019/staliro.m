...
for ii=1:staliro_opt.runs
    ...
    RUNSTATS.new_run();
    if nargout>1
        if strcmp(staliro_opt.optimization_solver, 'SOAR_Taliro_LocalGPs')
            if isempty(SOAR_run_flag)
                run_number = ii;
            else
                run_number = SOAR_run_flag;
            end
            [getRun, getHistory] = feval(staliro_opt.optimization_solver,InpRange,staliro_opt,run_number);        
