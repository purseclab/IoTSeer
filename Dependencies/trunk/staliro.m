...
% Check if model is a cell array, which would indicate conformance testing
...
%% Runtime statistics data collection
global RUNSTATS;
...
%% If running parameter estimation
...
%% Some basic checks on the formulas
if isfield(preds,'map2varIdx') && ~opt.MTL_Param_Spec 
    for ii = 1:length(preds)
        if ~isempty(preds(ii).map2varIdx)
            wrn_msg_str = [' The option MTL_Param_Spec is set to zero (inactive) while the predicate map has non-empty mapping map2varIdx to a search variable. See predicate ',num2str(ii),' which is predicate ',preds(ii).str,'.'];
            warning(wrn_msg_str)
        end
    end
end
...
for ii=1:staliro_opt.runs
    ...
    if staliro_opt.save_intermediate_results
        intermediateStaliroResults = struct('results', results,'history',history, 'lastFinishedRun',ii);
