...
% (C) 2015, Adel Dokhanchi, Arizona State University
	...
	properties
        ...
        %	versions. See staliro_blackbox.m for newer versions)
		%
		...
		% Enable specifications with parameters that depend on the falsification search variables.
        % [This feature is not always enabled since there is no clean interface for it yet. Hence,
        % the users should be confident that they understand how to use it.]
		% 
		%    Default option : MTL_Param_Spec = 0; (disabled)
		%
		% If MTL_Param is set to 1, then any predicates whose predicate map has a non-empty field 
        % 'map2varIdx' will replace its value with the value of the search variable from init_cond 
        % that corresponds to the index map2varIdx.
        % 
        % Example: Consider the requirement "always the output y should be less than 1.1 times the 
        % initial condition x0, i.e., y<=x0". Here, x0 is a search variable with range defined in the  
        % init_cond search range, e.g., x0 = [1.1 5.7]. Also, we assume that the output y is one 
        % dimensional.
        %
        %     MTL specification: 
        %           phi = '[] yLTEx'
        %     Predicate map:
        %           preds(1).str = 'yLTEx'; % the predicate name
        %           preds(1).A = 1;  preds(1).b = 1.1; % Represents the constraint A*y <= 1.1*x0
        %           preds(1).map2varIdx = 1; % Indicates that yLTEx contains a parameter and that 
        %                                    % the parameter (x0) corresponds to the first search 
        %                                    % variable (first row) of init_cond.
        %
        % Set the parameter MTL_Param_Spec to 0 to deactivate the feature.
        MTL_Param_Spec = 0;
