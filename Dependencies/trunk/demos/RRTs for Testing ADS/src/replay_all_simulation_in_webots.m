...
for ff_i = 1:length(interestings)
    load(['../log/', interestings{ff_i}]);
    ...
    [traj, matlab_traj] = get_traj_for_webots(configurations, final_config_id, config_history, ego, agent);
    cost_hist = get_cost_history_for_webots(final_config_id, config_history, ego, agent, configurations);

    if ispc
        target_dest = ['D:/git_repos/sim-atav-assembla/Webots_Projects/controllers/replay_matlab_trace/matlab_trajectory', num2str(ff_i),'.mat'];
    else
