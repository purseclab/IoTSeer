...
nb_ContPoints = [4, 8]; %incremental

InpSignal = ComputeInputSignals(stepTime, UPoint, opt.interpolationtype, nb_ContPoints, input_range, time, 0);

[T, XT, YT, LT,CLG,GRD] = model_test_case_many_cars(XPoint, time, stepTime, InpSignal);
traj = XT(:,[1:3,6:8,10:12,14:16,18:20]);
traj(:,1:3) = convert_x_to_webots(traj(:,1:3));
traj(:,4:6) = convert_x_to_webots(traj(:,4:6));
traj(:,7:9) = convert_x_to_webots(traj(:,7:9));
traj(:,10:12) = convert_x_to_webots(traj(:,10:12));
traj(:,13:15) = convert_x_to_webots(traj(:,13:15));
