function [cost, ind, cur_par, rob, ret_traj, hs] = Compute_Robustness(input)
...
if iscell(input)
    ...
    if length(input)>1
        ...
        warning('off','Simulink:Logging:LegacyModelDataLogsFormat');
        if nargout > 4
            [cost{1}, ind{1}, cur_par{1}, rob{1}, ret_traj{1}, hs{1}] = Compute_Robustness_Right(staliro_InputModel, staliro_InputModelType, input{1});
        else
            [cost{1}, ind{1}, cur_par{1}, rob{1}] = Compute_Robustness_Right(staliro_InputModel, staliro_InputModelType, input{1});
        end
        warning('on','Simulink:Logging:LegacyModelDataLogsFormat');
    end
    ...
    if (staliro_opt.stochastic == 1) && (staliro_opt.n_workers > 1)
        ...
        if iscell(staliro_InputModel)
            ...
            [cost1, ind1, ~, rob1] = Compute_Robustness_Right(staliro_InputModel{1},staliro_InputModelType{1}, input);
            [cost2, ~, ~, rob2] = Compute_Robustness_Right(staliro_InputModel{2},staliro_InputModelType{2}, input);
            ...
            if nargout > 4
                [cost, ind, cur_par, rob, ret_traj, hs] = Compute_Robustness_Right(staliro_InputModel, staliro_InputModelType , input);
            else
                [cost, ind, cur_par, rob] = Compute_Robustness_Right(staliro_InputModel, staliro_InputModelType , input);
            end
        end
    end
end
