...
%history = struct('rob',[],'samples',[],'cost',[],'traj',{});
...
[curVal, ~, tm_param, rob, traj, hs] = Compute_Robustness(curSample);
if ~isequal(opt.parameterEstimation,1)
...
for i = 2:nSamples
    ...
    if (mod(i,params.fRestarts)==0)
        ...
        [curVal1, ~, tm_param, rob, traj, hs] = Compute_Robustness(curSample1);
        if nargout>1
