...
%
%     Options when dp_taliro is called from staliro
...
%          Pred(i).map2varIdx : indicates that the predicate with name 
%              Pred(i).str contains a parameter value which will be 
%              replaced by an input search variable which corresponds to 
%              the search space init_cond of staliro. To activate the 
%              option, set the staliro option 'MTL_Param_Spec' to 1.
%              See staliro_options.MTL_Param_Spec.
%               
       ...
       rob = hydis(rob_temp);
        ...
        try
            ...
            if strcmp(msg, 'signed distance to the guard set is positive!')
                disp('[dp_taliro] Ignoring the following mx_dp_taliro error:')
                display(ME.message)
