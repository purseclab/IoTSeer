...
if isempty(staliro_mtlFormula) 
    % When an MTL spec is not provided
    
    if strcmp(inputModelType, 'hautomaton')
        % For compatibility with earlier much versions replace ~ with
        % dummy variables
        [~, ~, tmp_rob] = DistTrajToUnsafe([T STraj]', inputModel.unsafe);
    else
...
elseif ~isempty(staliro_mtlFormula)
    % When an MTL formula is provided
    ...
    % If parameter mining (estimation) is executed, then staliro_Polarity will be non-empty.
    % We need to find the type of the optimization problem and set the parameter values.
    ...
    % Prepare predicates when the specification depends on the search variables.
    if staliro_opt.MTL_Param_Spec && isfield(pred_tmp,'map2varIdx')
        for ii = 1:length(pred_tmp)
            if ~isempty(pred_tmp(ii).map2varIdx)
                pred_tmp(ii).b = pred_tmp(ii).b*XPoint(pred_tmp(ii).map2varIdx);
            end
        end
    end
...
%% Need to check whether parameter estimation is performed so we can check for the error condition
if ~isempty(staliro_Polarity)
...
%% normalize the parameters before they are returned to the stochastic optimizer
if staliro_opt.normalization == 1
...
%% Handle parallel execution issues
